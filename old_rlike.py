...
def pathfind(board):
    ...
    while empties:
        checklist = solvable(board)
        while checklist:
            for empty in empties:
                if empty not in checklist:
                    put(board, empty, STONE)
...
How many levels deep? (blank for 15): """, menu=True))
        ...
        animate(getlit(board, [
                (find(board, PLAYER), 10)], LEVEL), .300, data=data)
        while HP > 0:
                if LEVEL > len(LEVELS):
                        break
